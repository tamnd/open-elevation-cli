...
describes what users get. Pick a format with `-o`, or let open-elevation choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
open-elevation <command> -o table   # aligned columns for reading
open-elevation <command> -o jsonl   # one JSON object per line, for piping
open-elevation <command> -o json    # a single JSON array
open-elevation <command> -o csv     # spreadsheet friendly
open-elevation <command> -o tsv     # tab-separated
open-elevation <command> -o url     # just the URL column
open-elevation <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
open-elevation <command> --fields id,title,url
```
...
```bash
open-elevation <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
open-elevation <command>            # a table, because this is a terminal
open-elevation <command> | wc -l    # JSONL, because this is a pipe
```
